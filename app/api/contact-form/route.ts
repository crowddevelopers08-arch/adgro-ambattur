...
function generateFormDataString(leadData: LeadData): string {
  ...
  if (leadData.procedure) details.push(`Treatment: ${leadData.procedure}`);
...
//   if (leadData.userAgent) details.push(`Device: ${leadData.userAgent}`);
  ...
  try {
    ...
    const telecrmPayload = {
      ...
      actions: [
        ...
        {
          "type": "SYSTEM_NOTE",
          "text": `Lead Source: ${leadData.source || 'Adgor Hair Ambattur Website'}`
        },
        {
          "type": "SYSTEM_NOTE",
        ...
        {
          "type": "SYSTEM_NOTE",
          "text": `Treatment: ${leadData.procedure || 'Not specified'}`
        },
        {
          "type": "SYSTEM_NOTE",
 ...
 * This endpoint is used by the LeadsTable dashboard
 */
export async function GET(request: Request) {
  try {
    ...
    const syncStatus = searchParams.get('sync') || '' // 'synced' or 'unsynced'
    ...
    const transformedLeads = leads.map(lead => ({
      ...lead,
      treatment: lead.procedure, // Alias for frontend
